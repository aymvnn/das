...
export type Taal = "nl" | "ar";

type Vertaling = Record<string, string>;

// NL is de brontaal: het bepaalt de complete sleutelset. AR moet daarna
// (via `satisfies Record<Sleutel, string>`) exact dezelfde sleutels hebben —
// ontbreekt er één, dan faalt tsc in plaats van dat de site stilzwijgend
// terugvalt op het Nederlands.
const NL = {
  // --- UI / algemeen ---
...
} satisfies Record<Sleutel, string>;

const T: Record<Taal, Vertaling> = { nl: NL, ar: AR };

let _taal: Taal = "nl";
let _onWissel: (() => void) | null = null;
...
export function t(sleutel: string): string {
  const v: string | undefined = T[_taal][sleutel];
  if (v != null) return v;
  return T.nl[sleutel] ?? sleutel;
}

/** Interpoleer {naam}-plekhouders in een vertaalde string. */
export function tf(sleutel: string, waarden: Record<string, string | number>): string {
  return t(sleutel).replace(/\{(\w+)\}/g, (m, k) => {
    const v: string | number | undefined = waarden[k];
    return v != null ? String(v) : m;
  });
}

function pasTekstToe(root: ParentNode = document): void {
  root.querySelectorAll("[data-i18n]").forEach((el) => {
    const sleutel: string | null = el.getAttribute("data-i18n");
    if (sleutel) el.textContent = t(sleutel);
  });
  root.querySelectorAll("[data-i18n-html]").forEach((el) => {
    const sleutel: string | null = el.getAttribute("data-i18n-html");
    if (sleutel) el.innerHTML = t(sleutel);
  });
  root.querySelectorAll("[data-i18n-attr]").forEach((el) => {
    const raw: string | null = el.getAttribute("data-i18n-attr");
    if (!raw) return;
    raw.split(";").forEach((paar) => {
      const idx: number = paar.indexOf(":");
      if (idx === -1) return;
      const attr: string = paar.slice(0, idx).trim();
      const sleutel: string = paar.slice(idx + 1).trim();
      if (attr && sleutel) el.setAttribute(attr, t(sleutel));
    });
  });
}

async function laadArabischeFont(): Promise<void> {
...
export async function zetTaal(lang: string): Promise<void> {
  _taal = lang === "ar" ? "ar" : "nl";
  try {
...
export function initI18n(onWissel: () => void): void {
  _onWissel = onWissel;
  ...
  zetDocument();
  if (_taal === "ar") void laadArabischeFont();
  pasTekstToe();
}
